...
const COUNCILS = [
  ...
  { label: 'Limerick City and County Council', aliases: [
  ...
  { label: 'Waterford City and County Council', aliases: [
...
const CANDIDATE_STEMS = [
  'data cent', 'datacent', 'data hall', 'data campus', 'data storage',
  'data processing', 'server', 'hyperscale', 'colocation', 'co-location',
  'cloud computing', 'compute campus', 'digital infrastructure',
  'mission critical', 'ICT facil', 'ICT campus',
  'information technology facil', 'information technology campus',
  'computer facil', 'computer campus'
];
...
const CANDIDATE_CLAUSE = buildCandidateClause();

function arcgisJsonp(params, timeoutMs = 15000) {
  return new Promise((resolve, reject) => {
    const callbackName = `__irish_dc_v6_${Date.now()}_${jsonpCounter++}`;
...
function normaliseFeature(rawFeature, officialCouncil) {
  ...
  return {
    ...
    geometry: {
      type: 'Point',
      coordinates: [Number(rawFeature.geometry.x), Number(rawFeature.geometry.y)]
    },
    properties: {
...
function buildCouncilWhere(council) {
  const authorityClause = council.aliases
    .map(alias => `PlanningAuthority='${sqlEscape(alias)}'`)
    .join(' OR ');
  return `((${authorityClause}) AND ${CANDIDATE_CLAUSE})`;
}

async function queryCouncil(council, timeoutMs = 16000) {
  const response = await arcgisJsonp({
    where: buildCouncilWhere(council),
...
async function processCouncil(council) {
  try {
    const features = await queryCouncil(council, 15000);
    ...
    try {
      const features = await queryCouncil(council, 22000);
      mergeFeatures(features);
      councilsChecked += 1;
      if (features.length) enableControls();
      setStatus(`Checked ${councilsChecked} of 31 councils · ${data.length.toLocaleString('en-IE')} applications mapped…`);
      ...
      console.warn(`Council query failed: ${council.label}`, secondError);
      setStatus(`Checked ${councilsChecked} of 31 councils · ${data.length.toLocaleString('en-IE')} applications mapped…`);
      scheduleRender();
    }
  }
...
function downloadCsv() {
  const selected = filteredData();
  const headers = [
    'Project / location', 'Applicant', 'Planning authority', 'Application number',
    'Received date', 'Decision', 'Appeal', 'Site area (ha)', 'Floor area (m²)',
    'Development description', 'Official link'
  ];
  const rows = selected.map(feature => {
    const item = props(feature);
    return [
      item.project_name, item.applicant, item.planning_authority, item.application_number,
      item.received_date, item.decision, item.appeal, item.site_area_ha ?? '',
      item.floor_area_sqm ?? '', item.description, item.source_url
    ];
  });
...
async function initialise() {
  ...
  await runPool(COUNCILS, 5, processCouncil);
  enableControls();
  renderDashboard();
